...
public class HW2 {
  ...
  public static class Student{
    ...
    boolean hasChosen; // check if the students already have courses.
    ...
    private Student(int year, int id, int[] preference) {
        ...
        this.hasChosen = false;//initialize variable
    }
  ...
  private static void test3() {
    ...
    students[8] =  new Student(3, 33, new int[]{1, 2, 3});
    students[9] =  new Student(4, 1, new int[]{});
    students[10] =  new Student(3, 3, new int[]{1, 2, 3});
    students[11] =  new Student(1, 43, new int[]{1, 2, 3});
      ...
      if(sortList.get(j).preference.length>0){//inner loop : if student has preference
        in: for(int i = sortList.get(j).round;i<sortList.get(j).preference.length;i++){
          ...
          if(courses[sortList.get(j).preference[i]-1].available<courses[sortList.get(j).preference[i]-1].candidate.length){
            ...
            sortList.get(j).hasChosen = true; //we make sure the students get the course
            break in;
        ...
        if(sortList.get(j).hasChosen == false){ 
          // if the all preferred-courses of current student was not available, 
          //then we loop through the existing courses to check if there are still available spots
